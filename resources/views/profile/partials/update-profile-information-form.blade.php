<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            <x-input-label for="job_role_id" :value="__('Role')" />
            <select name="job_role_id" id="job_role_id" class="form-input rounded-md shadow-sm mt-1 block w-full">
                @foreach($jobRoles as $role)
                    <option value="{{ $role->id }}" @if($role->id == $user->job_role_id) selected @endif>{{ $role->name }}</option>
                @endforeach
            </select>
        </div>
